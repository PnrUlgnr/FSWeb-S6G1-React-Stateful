...
const[aktifProgramci,SetAktifProgramci]=useState(null);
	
  const oneCikaninIsmi = () => {
    return programcilar.find(item=>item.id==aktifProgramci).isim;
    // Bunu sona bırakın!
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: aktifProgramci == null ?'royalblue':"gold" ,// 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {SetAktifProgramci(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
